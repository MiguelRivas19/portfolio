...
export class AccountService {
  ...
  constructor(
      ...
      private currancyPipe : CurrencyPipe) { }
  ...
  purchase(stock: Stock): void {
      ...
      if (stock.price < this.balance) {
          ...
          this.calculateValue();
          this.cacheValues();
          this.alertService.alert(`You bought ${stock.symbol} for ` + this.currancyPipe.transform(stock.price, 'USD', true, '.2'),'success');
      } else {
    ...
    sell(index: number): void {
        ...
        if (stock) {
            ...
            this.calculateValue();
            this.cacheValues();
            this.alertService.alert(`You sold ${stock.symbol} for ` + this.currancyPipe.transform(stock.price, 'USD', true, '.2'),'success');
        } else {
            this.alertService.alert(`You do not own the ${stock.symbol} stocks`,'danger');
        }
    }
